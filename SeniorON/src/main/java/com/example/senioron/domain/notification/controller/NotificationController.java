...
public class NotificationController {
    ...
    public Response<NotificationHomeListResponse> getNotificationHome(
        ...
        return Response.ok(notificationService.getHomeSettings(user.getUsersId(), seniorId));
    }

    @Operation(summary = "부모님 기기 연결상태 조회", description = "선택한 시니어와 연결된 부모님(PARENT) 기기가 온라인인지 조회합니다. 부모/기기 정보가 없으면 false를 반환합니다.")
    ...
    public Response<ParentDeviceStatusResponse> getParentDeviceStatus(
        ...
        return Response.ok(notificationService.getParentDeviceStatus(user.getUsersId(), seniorId));
    }
